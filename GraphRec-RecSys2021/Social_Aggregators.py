...
class Social_Aggregator(nn.Module):
    ...
    def forward(self, nodes, to_neighs):
        embed_matrix = torch.empty(len(nodes), self.embed_dim).to(self.device)
        for i in range(len(nodes)):
            tmp_adj = to_neighs[i]
            num_neighs = len(tmp_adj)

            e_u = self.u2e.weight[tmp_adj].to(self.device) # fast: user embedding
            ...
            u_rep = self.u2e.weight[nodes[i]].to(self.device)
